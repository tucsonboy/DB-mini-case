...
FROM facilities
WHERE monthlymaintenance * .20 > membercost AND membercost > 0;
...
the member name. */
SELECT facilities.name, CONCAT(surname, " ", firstname) AS fullname
FROM (( bookings
    INNER JOIN members ON bookings.memid = members.memid) 
    INNER JOIN facilities ON bookings.facid = facilities.facid)
WHERE members.memid = bookings.memid AND facilities.facid IN (0,1)
...
SELECT facilities.name, CONCAT(members.surname, ' ', members.firstname) AS fullname, 
CASE
...
SELECT member, facility, cost FROM
	(SELECT
	CONCAT(members.firstname, ' ',members.surname) AS member, facilities.name as facility,
	CASE
	WHEN members.memid = 0
	THEN facilities.guestcost * bookings.slots
	ELSE facilities.membercost * bookings.slots
